...
void computeHog(int winSize,int nbin,map<string,vector<Rect>> &src, vector<vector<float>> &dst){
    ...
    for (it=src.begin(); it!=src.end(); it++){
        string key = it->first;
        const vector<cv::Rect> &values = it->second;
        Mat img = cv::imread(key,cv::IMREAD_COLOR);
        ...
        for(int j = 0; j < values.size(); j++){
            ...
            cvtColor(roi_img,gray_img, cv::COLOR_BGR2GRAY);
            ...
            auto scale_img_roi = scale_img(roi);
            hog.computeOne(scale_img_roi, dst[dst.size() - 1].data());
...
//            {
            ...
            /*применить*/
            dst.push_back(vector<float>(hog.getDescriptorSize(),0));
            auto flip_img_roi = flip_img(roi);
            hog.computeOne(flip_img_roi, dst[dst.size() - 1].data());
//            }
        }
    }
    cout << endl;
...
HOGEstimator::HOGEstimator(const std::string& folder)
    : m_hogTrainer(new HOG())
{
    Estimate(folder);
    //Train(folder);
    //m_hogTrainer->
...
void HOGEstimator::Train(const std::string& folder)
{
...
void HOGEstimator::Estimate(const std::string& folder)
{
    string prefix = "woodlogs_";
    string save_prefix = "B:/Work/Dataset/Dataset/";
    //string PATH = "D:\\foto\\HAWKwood\\Single Image Benchmark\\S.1 and S.2 real";
    string PATH = folder;


    std::fstream origStream(save_prefix + "all.txt"), goodStream(save_prefix + "good.txt"), badStream(save_prefix + "bad.txt"), xmlStream(save_prefix + "good.xml");
    ...
    for(int i = 0; i < txtFiles.size();i++){
        ...
        const std::string origName = save_prefix + "Original/image_" + std::to_string(i) + ".bmp";
        //cv::imwrite(origName, mat);

        xmlStream << "\t<image file='" << name + ".bmp" << "'>" << std::endl;
        for(int j = 0; j < rects.size(); j++){
            const cv::Rect &rect = rects[j];

            auto negRect = rect;
            negRect.x += rect.width / 2;
            negRect.y += rect.height / 2;
            ...
            if(     !(negRect.x >= 0 &&
                    negRect.y >= 0 &&
                    negRect.x + negRect.width <= mat.cols &&
                    negRect.y + negRect.height <= mat.rows)) continue;


            xmlStream << "\t\t<box top='" << rect.x << "' left='" << rect.y << "' width='" << rect.width << "' height='" << rect.height <<"'/>" << std::endl;
            //if(j % 10 == 0){

            //cv::imwrite(save_prefix + "Good/good_" + std::to_string(count) + ".bmp", mat(rect));
            //cv::imwrite(save_prefix + "Bad/bad_" + std::to_string(count) + ".bmp", mat(negRect));
            //goodStream << "good_" + std::to_string(count) << " " << name << " " << rect.x << " " << rect.y << " " << rect.width << " " << rect.height << std::endl;
            //badStream << "bad_"  + std::to_string(count) << " " << name << " "  << negRect.x << " " << negRect.y << " " << negRect.width << " " << negRect.height << std::endl;
            //}
            count++;
        }
